...
class StockMasterDB:
    ...
    def _init_db(self):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            # インデックスの作成
            cursor.execute('CREATE INDEX IF NOT EXISTS idx_sector ON stocks_master(sector);')
            cursor.execute('CREATE INDEX IF NOT EXISTS idx_market ON stocks_master(market);')
            cursor.execute('CREATE INDEX IF NOT EXISTS idx_active ON stocks_master(is_active);')
    ...
    def load_tse_stocks_from_excel(self, file_path: str) -> pd.DataFrame:
        ...
        try:
            ...
            # 上場市場の抽出（市場・商品区分から）
            df['market'] = df['market_product_category'].str.extract(r'(プライム|スタンダード|グロース)')
    ...
    def update_master_data(self) -> bool:
        ...
        try:
            ...
            with sqlite3.connect(self.db_path) as conn:
                ...
                # 既存データを非アクティブに設定
                conn.execute('UPDATE stocks_master SET is_active = 0, updated_at = ?', (current_time,))
                ...
                for _, row in df.iterrows():
                    ...
                    ''', (
                        ...
                        row['jquants_code'], True, current_time
                    ))
    ...
    def get_stocks_by_sector(self, sector: str, active_only: bool = True) -> pd.DataFrame:
        """
        業種で銘柄を取得
