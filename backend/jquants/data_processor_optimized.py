...
class JQuantsDataProcessorOptimized:
    ...
    def get_all_prices_for_past_5_years_to_db_optimized(self, db_path: str):
        ...
        for i in range(0, total_codes, self.batch_size):
            ...
            self.logger.info(f"Processing batch {batch_num}/{total_batches} ({len(batch_codes)} codes)")
            ...
            try:
                ...
                if batch_successful:
                    ...
                    total_records = sum(len(df) for _, df in batch_successful)
                    self.logger.info(f"Saved {total_records} records for {len(batch_successful)} codes")
                
            except Exception as e:
                self.logger.error(f"Error processing batch {batch_num}: {e}")
                failed_codes.extend(batch_codes)
        
        self.logger.info(f"Completed: {successful_codes} successful, {len(failed_codes)} failed")
        if failed_codes:
    ...
    def update_prices_to_db_optimized(self, db_path: str):
        ...
        self.logger.info("Getting last dates for all codes...")
        ...
        self.logger.info(f"{len(codes_to_update)} codes need updates")
        ...
        for from_date, group_codes in date_groups.items():
            self.logger.info(f"Processing {len(group_codes)} codes from {from_date}")
            ...
            for i in range(0, len(group_codes), self.batch_size):
                ...
                try:
                    ...
                    if batch_successful:
                        self.save_quotes_batch(db_path, batch_successful)
                        total_records = sum(len(df) for _, df in batch_successful)
                        self.logger.info(f"Updated {total_records} records for {len(batch_successful)} codes")
                    
                except Exception as e:
                    self.logger.error(f"Error processing batch: {e}")
                    failed_codes.extend(batch_codes)
        
        self.logger.info(f"Update completed: {successful_codes} processed, {updated_codes} updated, {len(failed_codes)} failed")
