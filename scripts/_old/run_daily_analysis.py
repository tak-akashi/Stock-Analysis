        ...
        with analysis_config.get_database_manager() as db_manager:
            ...
            if 'hl_ratio' in modules:
                ...
                try:
                    ...
                    # Calculate HL Ratio using the determined end_date
                    ...
                    if result_df is not None and not result_df.empty:
                        # Delete existing data for the target date to avoid duplicates
                        cursor = db_manager.results_conn.cursor()
                        cursor.execute("DELETE FROM hl_ratio WHERE Date = ?", (calc_end_date_str,))
                        db_manager.results_conn.commit()
                        ...
                        result_df.to_sql(
                            ...
                            if_exists='append',
                            index=False,
                            dtype={'Date': 'TEXT', 'Code': 'TEXT', 'HlRatio': 'REAL', 'MedianRatio': 'REAL', 'Weeks': 'INTEGER'}
                        )
